...
pub const TimerWheel = struct {
    ...
    pub noinline fn schedule(self: *Self, lifetime: usize, timer: *Timer) void {
        var delta = lifetime;
        const bucket_index = self.calc_bucket_index(delta, &timer.tick);
        timer.hook.unlink();
    ...
    pub fn tick_with_limit(self: *Self, limit: usize) usize {
        const current_tick = self.ticks;
        self.ticks += 1;

        // First, cascade everything on the higher levels down to the first level.
        var lvl: usize = self.cfg.levels - 1;
        var lvl_mask = (@as(u64, 1) << @intCast(u6, lvl * self.cfg.granularity_per_level_log2)) - 1;
        while (lvl > 0) {
            if ((current_tick & lvl_mask) == 0) {
                self._cascade_lvl(current_tick, lvl, @intCast(u6, lvl * self.cfg.granularity_per_level_log2));
            }

            lvl -= 1;
            lvl_mask >>= self.cfg.granularity_per_level_log2;
        }

        // Everything that had to be cascaded is now down on the first level, so we proceed with triggering callbacks.
        var count: usize = 0;
        self._trigger_lvl(current_tick, limit, &count, 0);
        return count;
    }
    fn _trigger_lvl(self: *Self, current_tick: u64, limit: u64, count: *u64, lvl: usize) void {
        ...
        const bucket_index = (lvl * self.cfg.buckets_per_level) + (current_tick & (self.cfg.buckets_per_level - 1));
    ...
    fn _cascade_lvl(self: *Self, current_tick: u64, lvl: usize, lvl_shift: u6) void {
        assert(lvl > 0);

        const bucket_index = (lvl * self.cfg.buckets_per_level) + ((current_tick >> lvl_shift) & (self.cfg.buckets_per_level - 1));
        const bucket = &self.buckets[bucket_index];

        while (!bucket.list.is_empty()) {
            const timer = bucket.pop();
            self.buckets[self.calc_index(@intCast(u8, lvl - 1), timer.tick)].list.append(&timer.hook);
        }
    }
    ...
    fn calc_bucket_index(self: *Self, delta: u64, abs_tick: *u64) usize {
        var lvl: u8 = 0;
        ...
        while (delta >= lvl_start) {
            ...
            if (lvl == self.cfg.levels) {
                ...
                return self._cutoff_bucket_index(abs_tick);
            }
        }

        abs_tick.* = self.ticks + delta;
        return self.calc_index(lvl, abs_tick.*);
    }
    fn _cutoff_bucket_index(self: *Self, abs_tick: *u64) usize {
        @setCold(true);
        abs_tick.* = self.ticks + self.cfg.max_lifetime;
        return self.calc_index(@intCast(u8, self.cfg.levels - 1), abs_tick.*);
    }
    inline fn calc_index(self: *Self, lvl: u8, abs_tick: u64) usize {
        ...
        const abs_index = (abs_tick >> shift);
    ...
    inline fn _wheel_max_lifetime(lvls: u4, lvl_size_log2: u3, rgran_log2: u3) u64 {
        ...
        // The max possible lifetime is one unit less than the cutoff.
        return (cutoff - 1);
    }
...
test "hierarchy" {
    ...
    // scheduled timer triggers at exactly the right time
    timers.schedule(32, &t.timer);
    try testing.expectEqual(@as(usize, 0), timers.tick_many(32));
    try testing.expectEqual(@as(usize, 0), t.count);
    try testing.expectEqual(@as(usize, 1), timers.tick_many(1));
    try testing.expectEqual(@as(usize, 1), t.count);
    ...
    timers.schedule(256, &t.timer);
    try testing.expectEqual(@as(usize, 1), timers.tick_many(257));
    try testing.expectEqual(@as(usize, 3), t.count);

    timers.schedule(320, &t.timer);
    try testing.expectEqual(@as(usize, 1), timers.tick_many(321));
    try testing.expectEqual(@as(usize, 4), t.count);

    timers.schedule(38, &t.timer);
    try testing.expectEqual(@as(usize, 1), timers.tick_many(39));
